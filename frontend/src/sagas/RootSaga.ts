...
import { fetchUserData, setToken, setUserPerms } from '../reducers/actions/userActions';
...
function* fetchProtectedData(token: string) {
    try {
        ...
        if (sessions.payload !== undefined) {
            yield put(
                setOwnSessionStatus(sessions.payload.running, sessions.payload.peopleCount, sessions.payload.startTime),
            );
        } else {
