...
export const toggleModal = (value: boolean) => action(TOGGLE_KEY_MODAL, { value });

export const setKeys = (keys: Key[]) => action(SET_KEYS, { keys });

export const setKeyTypes = (keyTypes: KeyType[]) => action(SET_KEY_TYPES, { keyTypes });
...
export const addKey = (token: string, data: Partial<Key>) => {
    return async (dispatch: Dispatch<any>) => {
        try {
            ...
            if (!isNumber(userId) || !isNumber(unionId) || !isString(description) || !isNumber(keyType)) {
                dispatch(errorMessage('Malformed request'));
            } else {
                const res = await KeyService.addKey(token, userId, unionId, keyType, description);
