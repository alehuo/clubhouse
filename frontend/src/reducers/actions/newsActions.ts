...
export const addNewspostToList = (newspost: Newspost) => action(ADD_NEWSPOST_TO_LIST, { newspost });

export const deleteNewspostFromList = (id: number) => action(DELETE_NEWSPOST_FROM_LIST, { id });

export const toggleNewsAddModal = (val: boolean) => action(TOGGLE_ADD_NEWSPOST_MODAL, { val });

export const setEditId = (id: number) => action(SET_NEWSPOST_EDIT_ID, { id });

export const toggleNewsEditModal = (val: boolean) => action(TOGGLE_EDIT_NEWSPOST_MODAL, { val });

export const setIsAdding = (val: boolean) => action(SET_IS_ADDING_NEWSPOST, { val });

export const setIsEditing = (val: boolean) => action(SET_IS_EDITING_NEWSPOST, { val });

export const setNewsposts = (newsPosts: Newspost[]) => action(SET_NEWSPOSTS, { newsPosts });
