...
describe('sessionReducer', () => {
    ...
    it('Action TOGGLE_SESSION_PAGE should set sessionPage value correctly (false)', () => {
        expect(sessionReducer(undefined, toggleSessionPage(false))).toMatchObject<Partial<SessionState>>({
            sessionPage: false,
        });
    });
    it('Action TOGGLE_SESSION_PAGE should set sessionPage value correctly (true)', () => {
        expect(sessionReducer(undefined, toggleSessionPage(true))).toMatchObject<Partial<SessionState>>({
            sessionPage: true,
        });
    });
});
