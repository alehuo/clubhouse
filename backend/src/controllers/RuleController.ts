    ...
    public routes(): express.Router {
        this.router.get('', async (req, res) => {
            try {
                ...
                return res.status(StatusCode.OK).json(MessageFactory.createResponse<Rule[]>(true, '', rules));
            } catch (err) {
                logger.error(err);
                return res
                    .status(StatusCode.INTERNAL_SERVER_ERROR)
        ...
        this.router.post('/swap/:ruleId1(\\d+)/:ruleId2(\\d+)', async (req, res) => {
            try {
                ...
                if (!rule1 || !rule2) {
                    return res.status(StatusCode.NOT_FOUND).json(MessageFactory.createError('Rule not found'));
                }
                ...
                if (saved1 && saved2) {
                    ...
                    return res.status(StatusCode.BAD_REQUEST).json(MessageFactory.createError('Error swapping rules'));
                }
            } catch (err) {
                logger.error(err);
                return res
                    .status(StatusCode.INTERNAL_SERVER_ERROR)
                    .json(MessageFactory.createError('Error swapping rules', err as Error));
            }
        });
        ...
        this.router.get('/:ruleId(\\d+)', async (req, res) => {
            ...
            try {
                ...
                return res.status(StatusCode.OK).json(MessageFactory.createResponse<Rule>(true, '', rule));
            } catch (err) {
                logger.error(err);
                return res
                    .status(StatusCode.INTERNAL_SERVER_ERROR)
