    ...
    public routes() {
        this.router.get('', JWTMiddleware, async (req, res) => {
            try {
                ...
                if (keys.every(isKey)) {
                    return res.status(StatusCode.OK).json(MessageFactory.createResponse<Key[]>(true, '', keys));
                }
                return res
        ...
        this.router.get('/:keyId(\\d+)', JWTMiddleware, async (req, res) => {
            ...
            try {
                ...
                if (!key) {
                    return res.status(StatusCode.NOT_FOUND).json(MessageFactory.createError('Key not found'));
                }
                if (isKey(key)) {
                    return res.status(StatusCode.OK).json(MessageFactory.createResponse<Key>(true, '', key));
                }
                return res
                    .status(StatusCode.INTERNAL_SERVER_ERROR)
                    .json(MessageFactory.createModelValidationError('Key'));
        ...
        this.router.post(
            '',
            RequestParamMiddleware<Key>('keyType', 'unionId', 'description', 'userId'),
            JWTMiddleware,
            async (req, res) => {
                const { keyType, unionId, description, userId }: Partial<Key> = req.body;
                if (!isString(description) || !isNumber(userId) || !isNumber(unionId) || !isNumber(keyType)) {
                    return res
                ...
                try {
                    ...
                    if (!dbUser) {
                        return res.status(StatusCode.BAD_REQUEST).json(MessageFactory.createError('User not found'));
                    }
                    ...
                    return res
                        .status(StatusCode.INTERNAL_SERVER_ERROR)
                        .json(MessageFactory.createError('Server error: Cannot add new key'));
                }
            },
        );
        ...
        this.router.delete('/:keyId(\\d+)', JWTMiddleware, async (req, res) => {
            ...
            try {
                ...
                return res.status(StatusCode.OK).json(MessageFactory.createMessage('Key deleted'));
            } catch (err) {
                logger.error(err);
                return res
                    .status(StatusCode.INTERNAL_SERVER_ERROR)
