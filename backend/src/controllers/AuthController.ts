...
const isTokenData = (
    ...
    if (tmp.data.permissions === undefined) {
        return false;
    }
    return typeof tmp.data.permissions === 'number';
};
    ...
    public routes() {
        this.router.post('', RequestParamMiddleware<DbUser>('email', 'password'), async (req, res) => {
            try {
                    ...
                    try {
                        ...
                        if (match) {
                            ...
                            return res.status(StatusCode.OK).json(
                                MessageFactory.createResponse<{ token: string }>(true, 'Authentication successful', {
                                    ...{ token },
                                }),
                            );
                        } else {
                        ...
                        return res
                            ...
                            .json(MessageFactory.createError('Server error: Cannot authenticate user', ex as Error));
                    }
                }
            } catch (err) {
                logger.error(err);
                return res
                    .status(StatusCode.INTERNAL_SERVER_ERROR)
                    .json(MessageFactory.createError('Server error: Cannot authenticate user', err as Error));
            }
        });
        ...
        this.router.get('/spotify', async (req, res) => {
            ...
            try {
                ...
                if (isTokenData(tokenData)) {
                    const permissions = tokenData.data.permissions;
        ...
        this.router.get('/spotify_callback', async (req, res) => {
            ...
            if (state === null || state !== storedState) {
                ...
                try {
                    ...
                    const response = await axios.post<SpotifyAuth>('https://accounts.spotify.com/api/token', null, {
                        headers,
                        params,
                    });
