    ...
    public routes() {
        this.router.get('', JWTMiddleware, async (req, res) => {
            try {
                ...
                return res
                    .status(StatusCode.INTERNAL_SERVER_ERROR)
                    .json(MessageFactory.createError('Internal server error: Cannot get all users', err as Error));
            }
        });
        ...
        this.router.get('/ownData', JWTMiddleware, async (req, res) => {
            try {
                ...
                if (!user) {
                    return res.status(StatusCode.NOT_FOUND).json(MessageFactory.createError('User not found'));
                } else {
                    if (!isDbUser(user)) {
                        return res
                            .status(StatusCode.INTERNAL_SERVER_ERROR)
                    ...
                    return res
                        .status(StatusCode.OK)
                        .json(MessageFactory.createResponse<User>(true, '', userFilter(user)));
                }
            } catch (ex) {
                logger.error(ex);
                return res
                    .status(StatusCode.INTERNAL_SERVER_ERROR)
                    .json(MessageFactory.createError('Server error: Cannot get a single user', ex as Error));
            }
        });
            ...
            try {
                ...
                if (userId !== routeUserId) {
                    return res
                        ...
                        .json(MessageFactory.createError('You can only edit your own information'));
                }
                const user = await UserDao.findOne(userId);
                if (!user) {
                    return res.status(StatusCode.NOT_FOUND).json(MessageFactory.createError('User not found'));
                    ...
                    if (email && user.email !== email) {
                        ...
                        if (usr) {
                            return res
                                .status(StatusCode.BAD_REQUEST)
                                .json(MessageFactory.createError('Email address is already in use'));
                        }
                    }
                    ...
                    if (
                        ...
                        errors.push('First name cannot be empty or longer than 255 characters');
                    }
                    if (
                        lastName &&
                        ...
                        errors.push('Last name cannot be empty or longer than 255 characters');
                    }
                    ...
                    if (
                        ...
                        errors.push('Password cannot be empty or shorter than 8 characters');
                    } else {
                    ...
                    if (errors.length === 0) {
                        ...
                        if (result) {
                            ...
                            return res
                                .status(StatusCode.OK)
                                .json(MessageFactory.createResponse<User>(true, '', userFilter(updatedUser)));
                        } else {
                            return res
                                .status(StatusCode.BAD_REQUEST)
                        ...
                        return res
                            .status(StatusCode.BAD_REQUEST)
                            .json(MessageFactory.createError('Error editing user information', undefined, errors));
                    }
                }
            } catch (ex) {
                logger.error(ex);
                return res
                    .status(StatusCode.INTERNAL_SERVER_ERROR)
                    .json(MessageFactory.createError('Server error: Cannot edit user', ex as Error));
            }
        });

        this.router.post(
            '',
            RequestParamMiddleware<DbUser>('email', 'firstName', 'lastName', 'password'),
            async (req, res) => {
                try {
                    const {
                        email,
                    ...
                    if (!isString(email) || !isString(firstName) || !isString(lastName) || !isString(password)) {
                        return res
                            .status(StatusCode.BAD_REQUEST)
                    ...
                    if (user) {
                        ...
                        if (
                            ...
                            errors.push('First name cannot be shorter than 2 or longer than 255 characters');
                        }
                        if (
                            !Validator.isLength(lastName, {
                                min: 2,
                            ...
                            errors.push('Last name cannot be shorter than 2 or longer than 255 characters');
                        }
                        ...
                        if (errors.length > 0) {
                            return res
                                .status(StatusCode.BAD_REQUEST)
                                .json(MessageFactory.createError('Error registering user', undefined, errors));
                        }
                        ...
                        return res
                            ...
                            .json(MessageFactory.createResponse<User>(true, '', userFilter(savedDbUser)));
                    }
                } catch (err) {
                    logger.error(err);
                    return res
                        .status(StatusCode.INTERNAL_SERVER_ERROR)
                        .json(MessageFactory.createError('Internal server error: Cannot add user', err as Error));
                }
            },
        );
            ...
            async (req, res) => {
                ...
                try {
                    ...
                    if (!user) {
                        ...
                        const calendarEvents = await CalendarEventDao.findCalendarEventsByUser(user.userId);
                        if (!calendarEvents.every(isCalendarEvent)) {
                            return res
                                .status(StatusCode.INTERNAL_SERVER_ERROR)
                                .json(MessageFactory.createModelValidationError('CalendarEvent'));
                        }
                        await Promise.all(calendarEvents.map(event => CalendarEventDao.remove(event.eventId)));
                        // Remove messages
                        ...
                        await Promise.all(messages.map(msg => MessageDao.remove(msg.messageId)));
                        // Remove newsposts
                        ...
                        await Promise.all(newsPosts.map(newsPost => NewsPostDao.remove(newsPost.postId)));
                        // Remove sessions
                        ...
                        await Promise.all(sessions.map(session => SessionDao.remove(session.sessionId)));
                    ...
                    return res
                        .status(StatusCode.INTERNAL_SERVER_ERROR)
                        .json(MessageFactory.createError('Internal server error: Cannot delete user', ex as Error));
                }
            },
        );
