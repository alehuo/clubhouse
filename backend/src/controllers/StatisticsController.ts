    ...
    public routes() {
        this.router.get('', JWTMiddleware, async (req, res) => {
            try {
                ...
                return res.status(StatusCode.OK).json(MessageFactory.createResponse<Statistics>(true, '', result));
            } catch (err) {
                logger.error(err);
                return res
                    .status(StatusCode.INTERNAL_SERVER_ERROR)
                    .json(MessageFactory.createError('Server error: Cannot get statistics', err as Error));
            }
        });
        ...
        this.router.get('/:userId(\\d+)', JWTMiddleware, async (req, res) => {
            try {
                ...
                const result = await StatisticsDao.findStatisticsFromUser(Number(req.params.userId));
                ...
                if (result) {
                    ...
                    return res.status(404).json(MessageFactory.createError('Statistics not found'));
                }
            } catch (err) {
                logger.error(err);
                return res
