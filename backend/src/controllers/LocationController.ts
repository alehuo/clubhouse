...
import { isLocation, Location, locationFilter, Permission } from '@alehuo/clubhouse-shared';
...
class LocationController extends Controller {
    ...
    public routes() {
        this.router.get('', JWTMiddleware, PermissionMiddleware(Permission.ALLOW_VIEW_LOCATIONS), async (req, res) => {
            try {
                ...
                return res
                    .status(StatusCode.INTERNAL_SERVER_ERROR)
                    .json(MessageFactory.createError('Server error: Cannot get all locations', err as Error));
            }
        });
                    ...
                    if (location) {
                        if (isLocation(location)) {
                            return res
                                ...
                                .json(MessageFactory.createResponse<Location>(true, '', locationFilter(location)));
                        }
                        return res
                            .status(StatusCode.INTERNAL_SERVER_ERROR)
                            .json(MessageFactory.createModelValidationError('Location'));
                    } else {
                        return res.status(StatusCode.NOT_FOUND).json(MessageFactory.createError('Location not found'));
                    }
                } catch (err) {
                    logger.error(err);
                    return res
                        .status(StatusCode.INTERNAL_SERVER_ERROR)
                        .json(MessageFactory.createError('Server error: Cannot get a single location'));
                }
            },
        );
        ...
        this.router.post(
            ...
            async (req, res) => {
                try {
                    ...
                    return res
                        .status(StatusCode.INTERNAL_SERVER_ERROR)
                        .json(MessageFactory.createError('Internal server error: Cannot add a new location'));
                }
            },
        );
            ...
            async (req, res) => {
                ...
                try {
                    ...
                    if (locations) {
                        ...
                        if (result) {
                            return res.status(StatusCode.OK).json(MessageFactory.createMessage('Location removed'));
                        } else {
                            return res
                                .status(StatusCode.BAD_REQUEST)
                    ...
                    return res
                        .status(StatusCode.INTERNAL_SERVER_ERROR)
                        .json(MessageFactory.createError('Server error: Cannot delete a location'));
                }
            },
        );
