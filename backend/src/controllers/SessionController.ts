...
class SessionController extends Controller {
    ...
    public routes() {
        ...
        this.router.get('/ongoing', JWTMiddleware, async (req, res) => {
            try {
                ...
                return res
                    ...
                    .json(MessageFactory.createResponse<Session[]>(true, '', sessions.map(sessionFilter)));
            } catch (err) {
                logger.error(err);
                return res
                    .status(StatusCode.INTERNAL_SERVER_ERROR)
                    .json(MessageFactory.createError('Server error: Cannot get ongoing sessions', err as Error));
            }
        });
        ...
        this.router.get('/ongoing/user/:userId(\\d+)', JWTMiddleware, async (req, res) => {
            try {
                const sessions = await SessionDao.findOngoingByUser(Number(req.params.userId));
                if (!sessions.every(isSession)) {
                    return res
                        .status(StatusCode.INTERNAL_SERVER_ERROR)
                        .json(MessageFactory.createModelValidationError('Session'));
                ...
                return res
                    ...
                    .json(
                        MessageFactory.createError(
                            'Server error:' + ' Cannot get current running sessions from a single user',
                            err as Error,
                        ),
                    );
            }
        ...
        // Start a session.
        this.router.post('/start', RequestParamMiddleware<Session>('startMessage'), JWTMiddleware, async (req, res) => {
            ...
            try {
                ...
                if (sessions && sessions.length > 0) {
                    return res
                        ...
                        .json(MessageFactory.createError('You already have an ongoing session running.'));
                }
                ...
                const title =
                    (process.env.MAIL_PREFIX ? '[' + process.env.MAIL_PREFIX + ']: ' : '') +
                    user.firstName +
                    ' ' +
                    user.lastName +
                ...
                try {
                    await ws.broadcastMessage(WsMessage(MessageType.SessionStart, req.body.startMessage, userId));
                } catch (err) {
                    logger.error(err);
                }

                return res.status(StatusCode.CREATED).json(MessageFactory.createMessage('Session started'));
            } catch (err) {
                logger.error(err);
                return res
                    .status(StatusCode.INTERNAL_SERVER_ERROR)
                    .json(MessageFactory.createError('Server error: Cannot start a session', err as Error));
            }
        });
        // Stop a session.
        this.router.post('/stop', RequestParamMiddleware<Session>('endMessage'), JWTMiddleware, async (req, res) => {
            ...
            try {
                ...
                if (sessions) {
                    if (sessions.length === 0) {
                        return res
                            .status(StatusCode.BAD_REQUEST)
                            .json(MessageFactory.createError("You don't have an ongoing session."));
                ...
                if (currentSession === undefined) {
                    return res.status(StatusCode.BAD_REQUEST).json(MessageFactory.createError('Invalid session'));
                }

                if (currentSession.sessionId === undefined) {
                    return res.status(StatusCode.BAD_REQUEST).json(MessageFactory.createError('Invalid session id'));
                }

                if (!isString(endMessage)) {
                    return res
                        .status(StatusCode.BAD_REQUEST)
                        .json(MessageFactory.createError('Invalid format for ending message'));
                }
                ...
                try {
                    await ws.broadcastMessage(WsMessage(MessageType.SessionEnd, req.body.endMessage, userId));
                } catch (err) {
                    logger.error(err);
                }
                ...
                return res
                    .status(StatusCode.OK)
                    .json(MessageFactory.createMessage("Session ended with message '" + req.body.endMessage + "'"));
            } catch (err) {
                logger.error(err);
                return res
                    .status(StatusCode.INTERNAL_SERVER_ERROR)
                    .json(MessageFactory.createError("Server error: Can't end a session", err as Error));
            }
        });
