...
import { CalendarEvent, isCalendarEvent, Permission } from '@alehuo/clubhouse-shared';
    ...
    public routes() {
        this.router.post(
            ...
            async (req, res) => {
                ...
                try {
                    ...
                    return res
                        .status(StatusCode.INTERNAL_SERVER_ERROR)
                        .json(MessageFactory.createError('Server error: Cannot add a new calendar event', ex as Error));
                }
            },
        );
        ...
        this.router.get('', async (req, res) => {
            try {
                ...
                return res
                    .status(StatusCode.INTERNAL_SERVER_ERROR)
                    .json(MessageFactory.createError('Server error: Cannot get all events', ex as Error));
            }
        });
        ...
        this.router.get('/ical', async (req, res) => {
            try {
                ...
                res.setHeader('Content-disposition', 'attachment; filename=events_all.ics');
        ...
        this.router.get('/:eventId(\\d+)', async (req, res) => {
            try {
                const event = await CalendarEventDao.findOne(Number(req.params.eventId));
                if (event === undefined) {
                    return res
                ...
                return res
                    .status(StatusCode.INTERNAL_SERVER_ERROR)
                    .json(MessageFactory.createError('Server error: Cannot get a single event', ex as Error));
            }
        });
            ...
            async (req, res) => {
                try {
                    ...
                    if (!event) {
                        ...
                        const result = await CalendarEventDao.remove(Number(req.params.eventId));
                        if (result) {
                            return res
                                .status(StatusCode.OK)
                            ...
                            return res
                                .status(StatusCode.INTERNAL_SERVER_ERROR)
                                .json(MessageFactory.createError('Failed to remove calendar event'));
                        }
                    }
                } catch (ex) {
                    logger.error(ex);
                    return res
                        .status(StatusCode.INTERNAL_SERVER_ERROR)
                        .json(MessageFactory.createError('Server error: Cannot delete a single event', ex as Error));
                }
            },
        );
        ...
        this.router.get('/:eventId(\\d+)/ical', async (req, res) => {
            try {
                const event = await CalendarEventDao.findOne(Number(req.params.eventId));
                if (!event) {
                    return res.status(StatusCode.NOT_FOUND).json(MessageFactory.createError('Event not found'));
                } else {
                    const calData = await createICal(event);
                    res.setHeader('Content-disposition', 'attachment; filename=event_' + event.eventId + '.ics');
                    res.setHeader('Content-type', 'text/calendar');
