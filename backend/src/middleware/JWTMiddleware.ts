...
export const JWTMiddleware: RequestHandler = async (req, res, next) => {
    ...
    if (authHeader === undefined) {
        ...
        return res.status(StatusCode.BAD_REQUEST).json(MessageFactory.createError('Missing Authorization header'));
    }
    ...
    if (headerParts.length === 2 && headerParts[0] === 'Bearer') {
        ...
        return res.status(StatusCode.BAD_REQUEST).json(MessageFactory.createError('Malformed Authorization header'));
    }
};
