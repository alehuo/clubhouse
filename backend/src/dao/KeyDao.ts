...
class KeyDao implements Dao<Key> {
    public findAll(): PromiseLike<Key[]> {
        return Promise.resolve(knex<Key>(TABLE_NAME).select());
    }

    public findOne(id: number): PromiseLike<Key | undefined> {
    ...
    public save(entity: Key): PromiseLike<number[]> {
        ...
        entity.updated_at = moment().format(dtFormat);
        return Promise.resolve(knex<Key>(TABLE_NAME).insert(entity));
    }
}
