...
describe('PermissionMiddleware', () => {
    it('Should return an error if the token is not set', async () => {
        ...
        PermissionMiddleware(Permission.ALLOW_ADD_EDIT_REMOVE_EVENTS)(request, response, function() {
            nextCalled += 1;
        });
    ...
    it('Should return an error if the user has no permissions', async () => {
        ...
        PermissionMiddleware(Permission.ALLOW_VIEW_STUDENT_UNIONS)(request, response, function() {
            nextCalled += 1;
        });
        expect(nextCalled).to.equal(0);
        const data: ApiResponse<undefined> = JSON.parse(response._getData());
    ...
    it('Should call next() if the user has permissions', async () => {
        ...
        PermissionMiddleware(Permission.ALLOW_ADD_REMOVE_KEYS)(request, response, function() {
            nextCalled += 1;
        });
